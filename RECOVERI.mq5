...
//|  v1.45                                                           |
//|  Добавлено в v1.45 (Mode 5 — выравнивание по смене тренда):      |
//|    - InpTrendFlipTrimStrong (default false): при флипе тренда    |
//|      советник суммирует объём ВСЕХ managed-позиций раздельно по  |
//|      BUY и SELL (PR-AVG-*, PR-LOCK, исходные, AVG-*, GRID-* —    |
//|      всё, что попадает в IsManaged), и выравнивает «сильную»     |
//|      сторону под объём «слабой»: режет diff = |BUY-vol -         |
//|      SELL-vol|, начиная с самых прибыльных позиций сильной       |
//|      стороны (lock-in профита), с частичным закрытием последней  |
//|      при необходимости.                                          |
//|      Это «общее откусывание» — оно ВКЛЮЧАЕТ ноги PR-LOCK как     |
//|      кандидатов на чип, в отличие от v1.44, где лок и оригиналы  |
//|      не трогались.                                               |
//|      Срабатывает в том же блоке, что InpRestartGridOnTrendFlip / |
//|      InpCloseOldGridOnTrendFlip, и не отменяет их: рестарт счёт- |
//|      чика на новой стороне и корзинное закрытие старой PR-AVG-   |
//|      сетки продолжают работать, а trim добавляется поверх как    |
//|      финальный шаг выравнивания. Цель: если рынок развернулся,   |
//|      не сидеть перекосом против нового тренда.                   |
...
#property strict
#property description "Universal MT5 Recovery EA v1.45 - Mode 5: trend-flip trims strong side to match weak side across all managed groups (incl. PR-LOCK)"
...
input bool               InpTrendFlipTrimStrong    = false;          // v1.45 Mode5: при смене тренда выровнять "сильную" сторону под объём слабой (режет все группы, включая PR-LOCK)
   ...
   if(trendFlipped)
     {
      ...
      // v1.45: trim the heavier side down to the lighter side's net
      // volume across ALL managed groups (PR-AVG-*, PR-LOCK-*, originals,
      // AVG-*, GRID-*). Runs right after the Restart/Close-old arming
      // and is independent of them — they still apply on top.  Only
      // kicks in when InpTrendFlipTrimStrong is enabled; runs once per
      // flip event.
...
//+------------------------------------------------------------------+
//| v1.45: sum the lot volume of all *managed* positions per side.    |
...
//+------------------------------------------------------------------+
//| v1.45: trend-flip trim — close winners on the heavier side until  |
//| BUY-vol approx SELL-vol.                                          |
//|                                                                   |
//| Triggered from the trend-flip block in DoPartialRecovery() when   |
...
//| PR-LOCK-*, originals, AVG-*, GRID-* — per the user's "across all  |
//| groups, including the lock leg" requirement (v1.44 only flushed   |
//| PR-AVG-*; this routine intentionally also reaches PR-LOCK).       |
//|                                                                   |
