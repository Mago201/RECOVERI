...
**Версия 1.43** — фиксы в `Mode 5` (PartialRecovery):
- **Суммарный итог цикла «averager TP + chip от лока» больше не уходит в минус.** Раньше при срабатывании TP по усреднителю советник всегда откусывал фиксированный `InpPartCloseLot` от убыточной позиции/лока — если позиция была в глубоком минусе, реализованный убыток от чипа превышал прибыль усреднителя, итог отрицательный. Теперь добавлена функция `AffordableChipLot()`: лот чипа ограничивается так, чтобы реализованный убыток не превышал прибыль только что закрытого усреднителя. Если даже минимальный лот не помещается в бюджет — чип пропускается и пишется в лог (попробуем на следующем TP усреднителя, когда профит накопится).
- **При смене тренда сетка меняет направление.** Раньше `InpUseTrendFilter` работал только как veto — блокировал усреднители против тренда, но не запускал в новую сторону при развороте. Теперь если тренд-фильтр включён и есть направление: BUY-усреднители открываются при `trend > 0`, SELL-усреднители при `trend < 0` (независимо от того, какая сторона сейчас убыточнее). При нейтральном тренде / выключенном фильтре — старая эвристика «открывать против убыточной стороны».
